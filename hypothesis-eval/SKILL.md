...
For each hypothesis, score across **5 dimensions** (see [evaluation-framework.md](evaluation-framework.md)):

1. **Testability** — Can it be proven wrong? (falsifiability, specificity, discriminability, feasibility)
2. **Evidence Quality** — Is it grounded in reality? (evidential support, groundedness, replicability)
3. **Explanatory Power** — Does it explain well? (scope, mechanism, unification, coherence)
4. **Productivity** — Will it advance knowledge? (novelty, progressiveness, fruitfulness)
5. **Robustness** — Can it survive scrutiny? (simplicity, auxiliary transparency, sensitivity, bias resistance, consistency)

Score each dimension **1-10** with explicit justification. Use confidence bands (low/moderate/moderate-high/high), not false-precision decimals. Do NOT score without evidence or reasoning.
...
[HYPOTHESIS]
1. Identify the 5 MOST CRITICAL auxiliary assumptions — those whose failure would most damage the hypothesis. Do not exhaustively enumerate; focus on the ones that matter.
2. Rate each: well-established / debatable / speculative
...
**Cross-model option**: If `mcp__openrouter__openrouter_chat` or `mcp__gemini-cli__ask-gemini` is available, route ONE of the three stress-test agents through an external model to break self-evaluation circularity. Prefer routing the Devil's Advocate — adversarial analysis benefits most from an independent perspective.
...
### 0. Epistemic Notice
Include at the top of every EVALUATION.md:
> **Epistemic notice**: This analysis was generated by an LLM evaluating LLM-generated hypotheses. Scores are ordinal assessments, not calibrated probabilities. Structured output format does not imply validated conclusions. For high-stakes decisions, seek independent expert evaluation.

### 1. Scorecard
Full 5-dimension scores per hypothesis, with overall average.
...
Compare structured evaluation against the naive baseline (Phase 1.5). Explicitly state:
- What did the structured evaluation surface that the baseline missed?
- If the baseline captured the same key points, note this honestly.
...
### 7. Bias Check
Flag any biases detected in the evaluation itself (see [evaluation-framework.md](evaluation-framework.md) bias checklist). In particular, check for the **Eloquent Consensus Trap** — does this evaluation create a false sense of rigor?
